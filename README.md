...
> UAC появится при регистрации `comcntr.dll`; скрипт можно запускать без «Запуск от имени администратора».  
> После UAC должно появиться окно **«DllRegisterServer в …\comcntr.dll выполнен успешно»** (скрипт вызывает regsvr32 **без** `/s`).
...
3. Ручная регистрация (PowerShell **от администратора**, 64-bit `regsvr32`, **не** SysWOW64):
