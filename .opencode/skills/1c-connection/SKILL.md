...
|---------|----------------------|
| `onec_com_status`: V83/V85 = false | **`Register-1CCom.cmd`** в **корне** `1c-analyst-tools` (поле `registerScriptPath`), подтвердить UAC — **не** «PowerShell от администратора» |
...
**Не требуется:** открывать 1С:Предприятие/Конфигуратор перед connect, PowerShell от администратора (только UAC при regsvr32).
