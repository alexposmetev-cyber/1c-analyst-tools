...
## Bridge Agent (рекомендуется для onec_query)

Долгоживущий COM-мост: стабильнее многократных `onec_query` через Get-1CData.ps1.

**Пользователь (один раз):**

1. `bridge\agent\bridge-agent.json` — копия из `bridge-agent.json.example`, та же ИБ что в `onec_connect`
...
**Агент:** `onec_bridge_status` после connect — `readyForQuery: true` → `onec_query` автоматически через Bridge (`via: bridge` в ответе).

Если Bridge не запущен — `onec_query` идёт через COM с полем `bridgeNote` в ответе.
...
3. `onec_list_infobases` → **question** по очереди: база → пользователь 1С → **пароль** (отдельный вопрос, даже если пустой).
4. При смене базы — `onec_disconnect` → новый connect.
5. **`onec_confirm_credentials(..., password_acknowledged=true)`** — только после явного ответа про пароль.
6. **`onec_connect(...)`** — те же user, база и password, что в confirm_credentials.
...
7. Ответ connect — русский текст. Дождитесь **конкретного** описания задачи → **`onec_declare_symptom`** (не «Ошибка в учёте»).
8. Без конкретного `declare_symptom` MCP **заблокирует** `its_search`, `search_cases`, `metadata_search`.
9. **`onec_refresh_metadata`** — после declare_symptom, отдельным вызовом.
